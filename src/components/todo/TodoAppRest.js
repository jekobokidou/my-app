...
class TodoAppRest extends Component {
...
class ListTasksComponent extends Component {
    ...
    handleDeleteTask(taskid){
        const username = "jekoboki";
        console.log(`http://localhost:8080/users/${username}/tasks/${taskid}`);
        axios.delete(`http://localhost:8080/users/${username}/tasks/${taskid}`)
            .then(response => this.handleTaskDeleted(response))
    }

    handleTaskDeleted(response){
        this.setState({
            message: "Delete of task successul"
        });
        this.refreshTaskList();
    }

    refreshTaskList(){
        axios.get('http://localhost:8080/users/jekoboki/tasks')
            .then(response => this.handleRetrieveTasks(response))
    }
                ...
                <table className="table">
                    ...
                    <tbody>
                        ...
                        <tr key={task.identifiant}>
                            ...
                            <td><button onClick={() => this.handleDeleteTask(task.identifiant)} className="btn btn-warning">Supprimer</button></td>
                        </tr>
                        )
                        }
                    </tbody>
...
class WelcomeParamComponent extends Component {
    ...
    constructor(props){
        ...
        this.handleGetHelloError = this.handleGetHelloError.bind(this);
    ...
    handleErrorHello(error){
        this.setState({
            helloMessage : error.response.data.message
        });
    }
    ...
    handleGetHelloError(){
        axios.get('http://localhost:8080/hello-world-error')
            .then(response => this.handleSuccessHello(response))
            .catch(error => this.handleErrorHello(error));
    }
    ...
    render (){
        return (
            <div className="container">
                ...
                <div className="container">
                    <button onClick={this.handleGetHelloError} className="btn btn-danger">Get Hello Error From Spring Boot</button>
                </div>
